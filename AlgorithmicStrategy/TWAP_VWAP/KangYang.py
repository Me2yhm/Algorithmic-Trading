...
def get_date_data(date: str, ticker: str, trade_delay=500):
    current_dir = Path(__file__).parent
    date_name = date[:4] + "-" + date[4:6] + "-" + date[6:]
    data_api = current_dir.parent / str(
        "datas/" + ticker + "/tick/gtja/" + date_name + ".csv"
    )
    ...
    writer_3s = Writer(
        "000157_3s_" + date[4:] + ".csv", None, rollback=3000, bid_ask_num=10
    )
