...
class TWAP(AlgorithmicStrategy):
    ...
    def __init__(self, orderbook: OrderBook,
                 ...
                 pre_close: float = 0,) -> None:
        super().__init__(orderbook, commision, stamp_duty, transfer_fee, pre_close)
        self.time_dict = {}
        self.tick = tick
        self.ob = orderbook
        self.signal = {'symbol':symbol,'direction':direction,'price':None,'volume':trade_volume/trade_num}
        self.ts_list = []
        self.trade: bool
    ...
    def signal_update(self):
        for (ts,action) in self.time_dict:
            if ts == self.timeStamp and action['trade']:
                ...
                self.vwap_market = list(self.ob.search_snapshot(ts)["total_trade"].values())[2]
                self.trade = True
                break
            else:
                self.trade = False
