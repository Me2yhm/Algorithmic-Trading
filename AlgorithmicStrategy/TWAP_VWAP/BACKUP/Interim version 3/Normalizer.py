...
class Normalizer:
    def __init__(self, folder_path: str, is_train: bool = True, **kwargs):
        ...
        self.continuous_var = kwargs.get(
            "continuous_var",
            [
                "preclose_range",
                "open_range",
    ...
    def get_csv_filenames(self):
        ...
        if self.is_train == True:
            ...
            self.df_his_feature = (
                self.df_his_feature - self.df_his_feature.mean()
            ) / self.df_his_feature.std()
            self.df_his_feature = pd.concat(
                [self.df_his_feature, df_VWAP_original], axis=1
            )
    ...
    def to_normalize(self, df):
        ...
        self.df_normalized = pd.concat(
            [df_supportive, df_cont, df_dis, self.df_his_feature, df_original], axis=1
        )
        return self.insert_columns(self.df_normalized)

    def initialize_output(
        self, is_train: bool = True, output_path: str = None, output: bool = True
    ):
        self.is_train == is_train
