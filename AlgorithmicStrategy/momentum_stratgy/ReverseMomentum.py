...
class T():#周期
    ...
    def cal_T(self):
        ...
        opentime = str(time.year)+str(time.month).zfill(2)+str(time.day).zfill(2)+'092500000'
...
class Hurst():
    ...
    def cal_hurst(self, i, ret_dict, period = 128):
        ...
        if (i-period+1) < 0: #周期不满
            ...
            if len(RS_new)!=0:
                self.hurst_dict[i] = np.polyfit(np.log(size_list_new), np.log(RS_new), 1)[0]
            else:
                self.hurst_dict[i] = None
...
class Factor1():
    ...
    def cal_factor1(self, i:int, ret:Ret, turnover:TurnOver, delta=5):
        ...
        if i - delta < 0: #周期不足 #要算ret需要下一个周期的price delta=5的时候需要0~4期（且第5期已到，即i>=5）的数据
            ...
            # breakpoint()
            sorted_turnover_periods = sorted([x for x in range(start_period, i) if turnover.dict[x] is not None], key=lambda x: turnover.dict[x])
            try:
                self.factor1_dict[i-1] = ret.dict[sorted_turnover_periods[-1]] -  ret.dict[sorted_turnover_periods[0]]
            except Exception as e:
...
class Factor2():
    ...
    def cal_factor2(self, i:int, ret:Ret, info:Info, delta=5):
        ...
        if i - delta  < 0: #周期不足            
            ...
            start_period =  i - delta
            sorted_info_periods = sorted([x for x in range(start_period, i) if info.dict[x] is not None], key=lambda x: info.dict[x])
            try:
                self.factor2_dict[i-1] = ret.dict[sorted_info_periods[-1]] -  ret.dict[sorted_info_periods[0]]
            except Exception as e:
...
class Model_reverse(modelType):
    ...
    def __init__(self, delta_stream=30000000,std_period=10, hurst_period=128, k=6, whole=294e10,delta_factor1=5,delta_factor2=5):
