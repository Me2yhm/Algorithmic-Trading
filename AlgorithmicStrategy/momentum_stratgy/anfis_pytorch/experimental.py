...
def linear_model(x, y, epochs=200, hidden_size=10):
    ...
    for t in range(epochs):
        ...
        perc_loss = 100.0 * torch.sqrt(tot_loss).item() / y.sum()
...
def plotResults(y_actual, y_predicted):
    ...
    """
    plt.plot(
        range(len(y_predicted)), y_predicted.detach().numpy(), "r", label="trained"
    )
...
def test_anfis(model, data, show_plots=False):
    ...
    print(
        "MS error={:.5f}, RMS error={:.5f}, percentage={:.2f}%".format(
            mse, rmse, perc_loss
        )
    )
    if show_plots:
...
def train_anfis(model: torch.nn.Module, data, epochs=500, show_plots=False):
    """
    Train the given model using the given (x,y) data.
    """
