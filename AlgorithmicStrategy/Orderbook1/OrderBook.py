...
class OrderBook:
    ...
    def _order_depth_update(self):
        ...
        self.last_snapshot["order_depth"]["weighted_average_depth"] = order_depth_calculator.calculate_weighted_average_depth(2)
    ...
    def _trade_update(self, data: OrderTick):
        ...
        try:
            ...
            if not self.candle_tick:
                ...
                None, data["price"], data["price"], data["price"], data["price"]
                ]         
                ...
                if timestamp not in self.candle_tick:
                    if len(self.snapshots.keys()) == 1:
                        ...
                        self.candle_tick[timestamp] = [
                        self.candle_tick[timestamp_last][4], data["price"], data["price"], data["price"], data["price"]
                        ]

                else:
                    candle = self.candle_tick[timestamp]
                    if data["price"] > candle[2]: candle[2] = data["price"]
                    if data["price"] < candle[3]: candle[3] = data["price"]     
                    candle[4] = data["price"]             
    ...
    def _get_avg_trade(self, timestamp_1, timestamp_2):
        ...
        if total_trade_2["passive_num"] == 0 :
            ...
            passive_stale_avg = total_trade_2["passive_stale_total"] / total_trade_2["passive_num"]
    ...
    def get_super_snapshot(self, level:int, timestamp:int):
        ...
        super_snapshot = dict()
        super_snapshot["ask_price"], super_snapshot["ask_volume"] = self.get_max_n_keys_and_values(ob["ask"], level)
        super_snapshot["bid_price"], super_snapshot["bid_volume"] = self.get_max_n_keys_and_values(ob["bid"], level)
        _, super_snapshot["ask_order_num"] = self.get_max_n_keys_and_values(ob["ask_num"], level)
        _, super_snapshot["bid_order_num"] = self.get_max_n_keys_and_values(ob["bid_num"], level)
        super_snapshot["ask_order_stale"] = self.get_values_from_ordered_dict(super_snapshot["ask_price"],
                                    self.divide_ordered_dicts(ob["ask_order_stale"], ob["ask_num_death"]))
        super_snapshot["bid_order_stale"] = self.get_values_from_ordered_dict(super_snapshot["bid_price"],
